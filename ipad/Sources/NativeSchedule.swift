...
struct NativeSchedule:View {
    ...
    private var controls:some View { HStack {
        ...
        Button {export=NativePDF.schedule(schedule,week:store.week,rows:rows,employees:store.employees)} label:{Label("Export",systemImage:"square.and.arrow.up")}.buttonStyle(.bordered).disabled(schedule.isNull)
    ...
    private func cell(_ row:J,_ day:String) -> some View {
        ...
        return Button {selected=ShiftSelection(row:row,day:day)} label: {
            ...
            }.padding(12).frame(width:150,minHeight:82,alignment:.leading).background(closed ? Color.secondary.opacity(0.04) : assigned.isEmpty ? Color.orange.opacity(0.05) : HOPStyle.band(row).opacity(0.06),in:RoundedRectangle(cornerRadius:12)).padding(4).contentShape(Rectangle())
...
struct NativeShiftEditor:View {
    ...
    var body:some View {NavigationStack {Form {
        ...
        if editable {Section("Assignment & time") {
            Toggle("Show staff outside availability / role match",isOn:$showAll)
            Picker("Employee",selection:Binding(get:{entry["employee_id"].text},set:{entry["employee_id"] = .s($0)})) {Text("Open shift").tag("");ForEach(candidates) {person in Text(person.displayName+" · "+person["role"].text).tag(person.id)}}
            ...
            else if let person=store.employees.first(where:{$0.id == entry["employee_id"].text}),BoardRules.off(person,day:selection.day,row:selection.row,availability:store.available) {ErrorNote(text:"This employee is marked off. The server will validate this assignment and publishing requires conflict review.")}
    ...
    private func save(_ value:J) {Task {do {_ = try await store.send("/api/schedules/draft/\(schedule.id)/entries",body:value);await store.load();dismiss()}catch {store.report(error)}}}
}
