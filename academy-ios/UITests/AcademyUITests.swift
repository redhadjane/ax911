...
final class AcademyUITests: XCTestCase {
    ...
    func testPracticeAnswerFeedbackAndResumeAfterRelaunch() {
        ...
        XCTAssertTrue(app.buttons["Check my answer"].isEnabled)
        app.buttons["Check my answer"].tap()
        print("AFTER PRACTICE ANSWER\n"+app.debugDescription)
        XCTAssertTrue(app.buttons["Next challenge"].waitForExistence(timeout:5))
        let feedback=app.staticTexts["study.feedback"];reveal(feedback)
        XCTAssertTrue(feedback.exists)
        let shot=XCTAttachment(screenshot:app.screenshot());shot.name="Practice feedback";shot.lifetime = .keepAlways;add(shot)
    ...
    func testMockAllowsFlagNavigationAndScoresOnlyOnSubmission() {
        ...
        XCTAssertTrue(app.staticTexts["study.position"].waitForExistence(timeout:12))
        let answer=app.buttons["answer.0"];reveal(answer);answer.tap()
        XCTAssertFalse(app.staticTexts["study.feedback"].exists)
        app.swipeDown()
